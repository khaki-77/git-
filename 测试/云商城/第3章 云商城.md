...
![1603446018961](images\1602391275025.png)
...
![1603446296664](images\1603446296664.png)
...
![1603448530004](images\1603448530004.png)
...
![1603451752022](images\1603451752022.png)



如下图：

![1603450922010](images\1603450922010.png)
...
修改本地文件`C:\Windows\System32\drivers\etc\HOSTS`文件，将案例演示域名`www.gpshopvip.com`解析到`192.168.100.130`服务器，在HOSTS文件中添加如下配置即可：
...
将`参考资料`中的`front`上传到`/usr/local/gupao/web/static`目录下，再修改`/usr/local/openresty/nginx/conf/nginx.conf`，配置如下：
...
访问`http://www.gpshopvip.com/`效果如下：

![1603458171418](images\1603458171418.png)
...
![1603526699620](images\1603526699620.png)
...
![1603532932980](images\1603532932980.png)
...
![1603541992869](images\1603541992869.png)
...
在`goods-api`中创建`com.gupaoedu.vip.mall.goods.model.AdItems`：
...
创建`com.gupaoedu.vip.mall.goods.mapper.AdItemsMapper`：
...
接口：修改`com.gupaoedu.vip.mall.goods.service.SkuService`,增加如下方法：
...
实现类：修改`com.gupaoedu.vip.mall.goods.service.impl.SkuServiceImpl`增加实现方法：
...
修改`com.gupaoedu.vip.mall.goods.controller.SkuController`增加方法：
...
用Postman测试`http://localhost:8081/sku/aditems/type/1`效果如下：

![1603545663780](images\1603545663780.png)
...
在`com.gupaoedu.vip.mall.goods.service.impl.SkuServiceImpl`添加`@Cacheable`注解，代码如下：

![1603548888435](images\1603548888435.png)
...
请求`http://localhost:8081/sku/aditems/type/1`此时Redis缓存数据如下：

![1603553882764](images\1603553882764.png)



我们可以发现上面存储的数据是二进制数据，我们很难阅读，而且占空间极大，我们可以使用FastJSON将每次存入到Redis中的数据转成JSON字符串，此时我们需要把`参考资料`中的`RedisConfig.java`拷贝到工程中，其他工程也有可能需要，我们可以拷贝到`mall-service-dependency`工程的`com.gupaoedu.vip.mall.config`包下。

此时清理再执行加载缓存后，效果如下：

![1603553850070](images\1603553850070.png)
...
接口：添加清理缓存方法`com.gupaoedu.vip.mall.goods.service.SkuService#delTypeSkuItems`
...
实现类：添加实现方法`com.gupaoedu.vip.mall.goods.service.impl.SkuServiceImpl#delTypeSkuItems`
...
添加删除缓存方法`com.gupaoedu.vip.mall.goods.controller.SkuController#deleteTypeItems`
...
在`goods-api`中创建`com.gupaoedu.vip.mall.goods.feign.SkuFeign`，代码如下：
...
![1603553512922](images\1603553512922.png)
...
访问`http://www.gpshopvip.com/sku/aditems/type?id=1`效果如下：
...
![1603554260125](images\1603554260125.png)
...
![1603558671591](images\1603558671591.png)
